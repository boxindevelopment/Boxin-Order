...
class CreateOrderTakePaymentsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('order_take_payments', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('order_detail_id')->unsigned();
            $table->integer('order_take_id')->unsigned();
            ...
            $table
                ->foreign('order_take_id')->references('id')->on('order_takes')
                ->onUpdate('cascade')->onDelete('cascade');
            $table
    ...
    {
        Schema::dropIfExists('order_take_payments');
    }
}
